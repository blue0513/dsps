...
If this configuration present, clients must present valid JWT with `Authorization: Bearer <jwt>` request header for every API call.

In addition, you can revoke JWT with [JWT revocation administration API](./interface/admin/revoke_jwt.md) if JWTs has `jti` (unique ID claim).
