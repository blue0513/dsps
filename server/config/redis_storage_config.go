...
func postprocessRedisSubStorageConfig(config *RedisStorageConfig) error {
	if config.IsSingleNode() && config.IsCluster() {
		return errors.New("Redis configration can have ONLY ONE of 'singleNode' and 'cluster' item, cannot specify both")
	}
	if !(config.IsSingleNode() || config.IsCluster()) {
		return errors.New("Redis configration must have one of 'singleNode' and 'cluster' item")
	}
