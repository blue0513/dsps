...
var (
	// ErrInvalidChannel : Given channel is not defined or is disabled in configuration
	ErrInvalidChannel = errors.New("storage.invalid-channel")
	// ErrSubscriberNotFound : Subscriber expired (due to infrequent access less than expire setting) or not created
	ErrSubscriberNotFound = errors.New("storage.subscriber-not-found")
	// ErrMalformedAckHandle : Failed to decode/decrypt given AckHandle
	ErrMalformedAckHandle = errors.New("storage.ack-handle-malformed")
)
...
func IsStorageNonFatalError(err error) bool {
	return errors.Is(err, ErrInvalidChannel) || errors.Is(err, ErrSubscriberNotFound) || errors.Is(err, ErrMalformedAckHandle)
}
...
type PubSubStorage interface {
	...
	FetchMessages(ctx context.Context, sl SubscriberLocator, max int, waituntil Duration) ([]Message, AckHandle, error)
